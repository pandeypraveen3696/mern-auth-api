...
exports.signup = (req, res) => {
  ...
  User.findOne({ email }).exec((err, user) => {
    ...
    sgMail
      .send(emailData)
      .then(sent => {
        // console.log("SIGN EMAIL SENT 1  ", sent);
        return res.json({
          message: `Email has been sent to ${email}. Follow the instruction to activate your account`
        });
      })
      .catch(err => {
        // console.log('Error Msg sign up email sent ',err)
        return res.json({
          message: err.message
        });
      });
            ...
            user.save((err, data) => {
              if (err) {
                ...
                return res.statue(400).json({
                  error: "USER Sign up failed up with google"
                });
              }
              const token = jwt.sign(
                { _id: data._id },
          ...
          if (user) {
            ...
            user.save((err, data) => {
              if (err) {
                ...
                return res.statue(400).json({
                  error: "USER Sign up failed up with facebook"
                });
              }
              const token = jwt.sign(
                { _id: data._id },
